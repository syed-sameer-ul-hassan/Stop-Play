...
var SETTINGS_KEY = "stop_play_settings";
var TAB_STATES_KEY = "stop_play_tab_states";
...
async function getStoredTabStates() {
  return new Promise((resolve) => {
...
async function saveStoredTabStates(states) {
  return new Promise((resolve) => {
    if (typeof chrome === "undefined" || !chrome.storage || !chrome.storage.local) {
      resolve();
      return;
...
var tabMediaMap = /* @__PURE__ */ new Map();
var lockedTabs = /* @__PURE__ */ new Set();
var currentActiveTabId = null;
var currentWindowId = null;
var notificationTabMap = /* @__PURE__ */ new Map();
...
async function handleBackgroundMediaAlert(tabId, trigger) {
  ...
  const now = Date.now();
  const cooldownMs = (settings.cooldownSeconds || 45) * 1e3;
  ...
  const notificationTitle = settings.autoPauseOnSwitch ? "Video Auto-Paused" : "Video Playing in Background";
  const notificationMessage = `${entry.state.title || "Video"} \u2022 ${domain}`;
  const contextMessage = settings.autoPauseOnSwitch ? "Auto-resumes when you return" : "Stop Play Guardian";
  chrome.notifications.create(
    ...
    (createdId) => {
      ...
      const timeout = (settings.notificationTimeoutSeconds || 5) * 1e3;
      setTimeout(() => {
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === "TEST_NOTIFICATION") {
    ...
    chrome.notifications.create(testId, {
      ...
      setTimeout(() => chrome.notifications.clear(testId), 6e3);
    });
